...
//!
//! 运行: cargo run -p nexus-prime --example diag_voice_tap
//! 退出码 1 = RED（复现到「松不开」）
...
fn force_release_sendinput(vks: &[u16]) {
    #[cfg(windows)]
    {
        ...
        let _ = nexus_prime_lib::bridges::xiaomi::hid_injector::release(vks);
    }
    #[cfg(not(windows))]
    {
        let _ = vks;
...
fn check_case(name: &str, vks: &[u16], hold_ms: u64) -> bool {
    ...
    nexus_prime_lib::bridges::xiaomi::key_mapping::tap_vks(vks, hold_ms);
    ...
    if !ok {
        ...
        let _ = nexus_prime_lib::bridges::xiaomi::hid_injector::release(&[0x74]);
...
fn main() {
    ...
    println!(
        "WinUHid available={}",
        nexus_prime_lib::bridges::xiaomi::hid_injector::is_available()
    );
    ...
    for i in 0..20 {
        nexus_prime_lib::bridges::xiaomi::key_mapping::tap_vks(&[0x41], 70);
