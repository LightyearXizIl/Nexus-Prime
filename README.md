...
- NSIS：`Nexus Prime_0.1.6_x64-setup.exe`
...
7. 确保输入法听写快捷键与 Nexus Prime 中的语音键映射一致；
8. 若音频无波形或出现“ATVV 未连接”，使用“修复 ATVV 连接”；
9. 关闭窗口时可按设置最小化到托盘，之后从托盘重新打开或退出。

首次启用 HID Tap 或安装虚拟声卡时，Windows 可能显示 UAC 提权提示。
...
应用配置和日志写入 Windows 应用数据目录，不应存放在源码仓库。分享日志前请删除蓝牙地址、用户名、绝对路径、进程信息和其他设备标识。
...
检查首页音频信号和语音路由状态，确认 VB-CABLE 已安装，并把输入法麦克风设置为 `CABLE Output`。再核对输入法听写快捷键与语音键映射。
